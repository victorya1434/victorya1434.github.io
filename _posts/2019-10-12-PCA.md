...
the final thing of pca we want to find out is the $$D$$, but the first thing we need to do is figure out how to generate the optimal code point $$c^{*}$$ for each input point $$x$$: $$C^{\*} = \arg\min_{C} \||X^{T} - DC^{T}|\|_2 ~~~~~~(1)$$
and the result of $$(1)$$:
\\[C^{T^{\*}} = D^{T}X^{T}\\]
